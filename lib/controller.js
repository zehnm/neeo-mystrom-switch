...
module.exports = (myStromService, discoveryController) => {
  ...
  function sendNotificationToBrain(uniqueDeviceId, component, value) {
    logger.debug('[CONTROLLER] Send notification to brain:', uniqueDeviceId, component, value);
    sendComponentUpdate({ uniqueDeviceId, component, value })
      .catch((error) => {
        // filter duplicate message errors: SDK keeps the last value and only sends changed data to the brain. If value doesn't change this error is returned.
        if (error.message !== 'DUPLICATE_MESSAGE') {
          logger.debug('[CONTROLLER] Sending notification to brain failed:', error.message);          
        }
      });
  }
