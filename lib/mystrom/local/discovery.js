...
/**
 * Auto discovery of myStrom devices on local subnet.
 * 
...
class MyStromLocalDiscovery extends EventEmitter {
  ...
  start() {
    ...
    this.socket.on('listening', () => {
      ...
      logger.info(`Listening for myStrom UDP broadcast on ${address.address}:${address.port}`);
      this.emit('start');
    });
    ...
    this.socket.on('message', (msg, rinfo) => {
      if (rinfo.size !== 8) {
        logger.warn('Ignoring invalid message of size: %d. Message:', rinfo.size, msg.toString('hex'));
        return;
      }
