  ...
  getState() {
    return new BluePromise((resolve, reject) => {
      ...
      request("http://" + this.host + "/report", function (error, response, body) {
        if (error) {
          console.log('[MyStromSwitch] Error getting state %s: %s', self.name, error);
          reject(error);
          ...
          console.log('[MyStromSwitch] %s state: %s, %s W', self.name, result.relay === true ? 'On' : 'Off', result.power);
          resolve(result);
        }
      });
    });
