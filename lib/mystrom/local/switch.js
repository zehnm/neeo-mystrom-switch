  ...
  getState() {
    return new BluePromise((resolve, reject) => {
      ...
      request("http://" + this.host + "/report", function (error, response, body) {
        if (error) {
          logger.error('[MyStromSwitch] Error getting state %s: %s', self.name, error);
          reject(error);
        } else if (response.statusCode !== 200) {
          reject(new Error('INVALID_ANSWER from ' + self.name + ': ' + response.statusCode + ' ' + response.statusMessage));
        } else {
          var result = JSON.parse(body);
          if (result.hasOwnProperty('power') && result.power != 0) {
            result.power = result.power.toFixed(1);
          }
          logger.debug('[MyStromSwitch] %s state: %s, %s W', self.name, result.relay === true ? 'On' : 'Off', result.power);
          resolve(result);
        }
      });
    });
